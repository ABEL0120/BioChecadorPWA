...
export const Home: React.FC = () => {
  ...
  const handleVerificarRfc = async (e?: React.FormEvent) => {
    //si no encuentra el rfc que no cargue los datos de la sucursal y solo muestre el toast del error
    ...
    try {
      ...
      if (response.success && response.data && response.data.existe) {
        ...
        if (response.data.ultimoMovimientoHoy === "ENTRADA") {
  ...
  const handleMarcarAsistencia = async () => {
    ...
    try {
      ...
      if (
        ...
        setResultado({ ...resultado, tieneBiometria: false });
        setToastState({
          show: true,
          message: "Credencial biométrica no encontrada o cancelada. Vuelve a registrar tu huella/rostro.",
          color: "warning",
        });
      } else {
        setToastState({
    ...
    <IonPage className="bg-slate-100">
      ...
      <IonContent className="ion-padding" style={{ "--background": "#f8fafc" }}>
        ...
        <div className="max-w-4xl mx-auto space-y-6 py-2">
          <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-blue-600 via-blue-600 to-blue-600 p-6 text-white shadow-lg shadow-blue-500/15">
                      ...
                      <div className="space-y-4 bg-white p-5 rounded-2xl border border-emerald-200 shadow-sm">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                          ...
                          <div className="px-4 py-2 bg-slate-100 rounded-xl flex items-center justify-center font-black text-sm text-slate-700 tracking-widest uppercase">
                            <IonIcon 
                              icon={tipoMovimiento === "ENTRADA" ? logInOutline : logOutOutline} 
